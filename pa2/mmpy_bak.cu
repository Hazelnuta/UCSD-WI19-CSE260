...
// #endif
	...
	// cudaMalloc((void **)&d_nA, sizeof(double) * SQR(rnd(n, BLOCK_SIZE)));
	// cudaMalloc((void **)&d_nA, sizeof(double) * SQR(rnd(n, BLOCK_R)));
	// cudaMalloc((void **)&d_nA, sizeof(double) * SQR(rnd(n, BLOCK_N)));
    checkCUDAError("Error allocating device memory for matrix A");
	// cudaMalloc((void **)&d_nB, sizeof(double) * SQR(rnd(n, BLOCK_SIZE)));
	// cudaMalloc((void **)&d_nB, sizeof(double) * SQR(rnd(n, BLOCK_R)));
	// cudaMalloc((void **)&d_nB, sizeof(double) * SQR(rnd(n, BLOCK_N)));
    checkCUDAError("Error allocating device memory for matrix B");
	// cudaMalloc((void **)&d_nC, sizeof(double) * SQR(rnd(n, BLOCK_N)));
	// cudaMalloc((void **)&d_nC, sizeof(double) * SQR(rnd(n, BLOCK_SIZE)));
	// cudaMalloc((void **)&d_nC, sizeof(double) * SQR(rnd(n, BLOCK_R)));
    checkCUDAError("Error allocating device memory for matrix C");
	...
	// bind_texture(n, h_A, h_B);
	// padding << < SQR(rnd(n, BLOCK_N)) / 1024, 1024 >> > (n, rnd(n, BLOCK_N), d_A, d_nA);
	// padding << < SQR(rnd(n, BLOCK_N)) / 1024, 1024 >> > (n, rnd(n, BLOCK_N), d_B, d_nB);
	// padding << < SQR(rnd(n, BLOCK_SIZE) / BLOCK_SIZE), SQR(BLOCK_SIZE) >> > (n, rnd(n, BLOCK_SIZE), d_A, d_nA);
	// padding << < SQR(rnd(n, BLOCK_SIZE) / BLOCK_SIZE), SQR(BLOCK_SIZE) >> > (n, rnd(n, BLOCK_SIZE), d_B, d_nB);
	// padding << < SQR(rnd(n, BLOCK_R) / BLOCK_C), SQR(BLOCK_C) >> > (n, rnd(n, BLOCK_R), d_A, d_nA);
	// padding << < SQR(rnd(n, BLOCK_R) / BLOCK_C), SQR(BLOCK_C) >> > (n, rnd(n, BLOCK_R), d_B, d_nB);
    cudaDeviceSynchronize();

    checkCUDAError("Error in padding kernel");
...
// shared memory, we may run slightly faster on devices that have the capability
    ...
    for (int r = 0; r < SCALE * reps; r++)
		// for (int r = 0; r < 1; r++)
		// matMul(n, d_C);
		// g_matMulo << < SQR((n + 15) / 16), 16 * 16 >> > (n, d_C, d_A, d_B);
        matMul << < grid, threads >> > (n, d_C, d_A, d_B);
	// g_matMul << < SQR(rnd(n, BLOCK_SIZE) / BLOCK_SIZE), SQR(BLOCK_SIZE) / BLOCK_PORTION >> > (rnd(n, BLOCK_SIZE), rnd(n, BLOCK_SIZE) / BLOCK_SIZE, d_nC, d_nA, d_nB);
	// g_matMulVD << < dim3(rnd(n, BLOCK_R) / BLOCK_R, rnd(n, BLOCK_R) / BLOCK_C, 1), BLOCK_R >> > (rnd(n, BLOCK_R), d_nC, d_nA, d_nB);
	...
	// unpadding << < SQR(rnd(n, BLOCK_N)) / 1024, 1024 >> > (n, rnd(n, BLOCK_N), d_C, d_nC);
	// unpadding << < SQR(rnd(n, BLOCK_SIZE) / BLOCK_SIZE), SQR(BLOCK_SIZE) >> > (n, rnd(n, BLOCK_SIZE), d_C, d_nC);
	// unpadding << < SQR(rnd(n, BLOCK_R) / BLOCK_C), SQR(BLOCK_C) >> > (n, rnd(n, BLOCK_R), d_C, d_nC);
...
clean:
	...
	// assert(cudaSuccess == cudaFree(d_nA));
	// assert(cudaSuccess == cudaFree(d_nB));
